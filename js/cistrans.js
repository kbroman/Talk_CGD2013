  ...
  draw = function(data) {
    ...
    draw_probe = function(probe_data) {
      ...
      yaxis.selectAll("empty").data(ticks).enter().append("line").attr("y1", function(d) {
        ...
        return lodcurve_yScale(d);
      }).attr("x1", left[1]).attr("x2", right[1]).attr("stroke", "white").attr("stroke-width", "2");
      ...
      yaxis.append("line").attr("y1", lodcurve_yScale(5)).attr("y2", lodcurve_yScale(5)).attr("x1", left[1]).attr("x2", right[1]).attr("stroke", purple).attr("stroke-width", "2").attr("stroke-dasharray", "3,3");
      lodcurve = function(c) {
      ...
      }).attr("r", bigRad).attr("fill", purple).attr("stroke", "none").attr("stroke-width", "1").attr("opacity", 0).on("mouseover", function(td) {
        if (!markerClick[td]) {
      ...
      draw_pxgXaxis = function(means, genotypes, chr, sexcenter, male) {
        ...
        }).attr("stroke", darkGray).attr("fill", "none").attr("stroke-width", "2");
        ...
        meanmarks.selectAll("line.PXGmeans").data(means).enter().append("line").attr("class", "PXGmeans").attr("x1", function(d, i) {
          ...
          return svg.selectAll("#efftip").remove();
        });
      ...
      plotPXG = function(marker) {
        ...
        pxgYaxis.selectAll("empty").data(pxgticks).enter().append("line").attr("y1", function(d) {
          ...
          return pxgYscale(d);
        }).attr("x1", left[2]).attr("x2", right[2]).attr("stroke", "white").attr("stroke-width", "2");
    ...
    peaks = svg.append("g").attr("id", "peaks").selectAll("empty").data(data.peaks).enter().append("circle").attr("class", function(d) {
      ...
      svg.selectAll("circle.probe_" + d.probe).attr("r", bigRad).attr("fill", pink).attr("stroke", darkblue).attr("stroke-width", 1).attr("opacity", 1);
      return eqtltip.call(this, d);
    }).on("mouseout", function(d) {
      svg.selectAll("circle.probe_" + d.probe).attr("r", peakRad).attr("fill", function(d) {
        if (chrindex[d.chr] % 2 === 0) {
          return darkblue;
        } else {
          return darkgreen;
      ...
      return svg.selectAll("#eqtltip").remove();
    }).on("click", function(d) {
